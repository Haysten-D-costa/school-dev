            ...
            return (
              ...
              >
                ...
                <div className="w-16 shrink-0 bg-primary flex flex-col items-center justify-center py-4 px-2 text-center">
                  <span className="font-heading font-black text-cream text-2xl leading-none">{day}</span>
                  <span className="font-body text-cream/70 text-[10px] tracking-[0.15em] uppercase mt-1">{month}</span>
                  <span className="font-body text-cream/40 text-[10px] mt-0.5">{year}</span>
                </div>

                {/* Content */}
                <div className="p-4 flex flex-col flex-1 min-w-0">
                  <span className={`inline-block self-start text-[10px] font-body font-bold tracking-[0.12em] uppercase px-2 py-0.5 rounded-sm mb-2 ${CATEGORY_COLORS[event.category]}`}>
                    {CATEGORY_LABELS[event.category]}
                  </span>
                  <h3 className="font-heading font-bold text-primary text-base leading-snug mb-2">
                    {event.title}
                  </h3>
                  <p className="font-body text-neutral/60 text-[13px] leading-relaxed flex-1 mb-3 line-clamp-2">
                    {event.excerpt}
                  </p>
                  {event.time && (
